...
app.post('/api/applications/:trackingNumber/messages', upload.array('attachments', 5), async (req, res) => {
  try {
    ...
    if (role === 'employer' && !admin && (!senderEmail || senderEmail !== cleanEmail(application.employer_email))) {
    ...
    const message = chatMessageData(application.id, req.body, {
      senderRole: role,
      senderEmail,
      senderName: role === 'applicant' ? application.full_name : role === 'admin' ? 'FindAJob.qa Support' : 'Employer',
    });
    ...
    if (error) return res.status(500).json({ error: error.message });
    if (role === 'applicant' && message.faq_key && FAQ_REPLIES[message.faq_key]) {
      await supabase.from('chat_messages').insert([{
        application_id: application.id,
        sender_role: 'system',
        sender_name: 'FindAJob.qa Assistant',
        body: FAQ_REPLIES[message.faq_key],
        is_encrypted: false,
...
app.get('/api/employers/applications/:id', async (req, res) => {
  ...
  const [{ data: comments }, { data: documents }, { data: messages }, { data: presence }] = await Promise.all([
...
const pageAliases = {
  ...
  '/career-advice.html': 'career-advice.html',
  '/privacy-policy.html': 'privacy-policy.html',
  '/terms-of-service.html': 'terms-of-service.html',
  '/admin.html': 'admin.html',
  '/apply-agent.html': 'contact.html',
};
