...
function createEmployerToken(email) {
  const payload = Buffer.from(JSON.stringify({ email: cleanEmail(email), exp: Date.now() + 1000 * 60 * 60 * 12 })).toString('base64url');
...
app.get('/api/employers/applications/:id', async (req, res) => {
  ...
  if (!email) return res.status(401).json({ error: 'Employer login required.' });
