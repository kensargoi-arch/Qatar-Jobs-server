...
// ─── Supabase client ────────────────────────────────────────────────────────
const SUPABASE_URL = process.env.SUPABASE_URL || 'https://ihreycbdwsiyhoesinxi.supabase.co';
const SUPABASE_KEY = process.env.SUPABASE_SERVICE_ROLE_KEY
  || process.env.SUPABASE_SECRET_KEY
  || process.env.SUPABASE_KEY
  || 'sb_publishable_jjSXNYgqPQCIYiYUsvnLsg_EshNThDI';
...
app.post('/api/applications', upload.fields([
  ...
  try {
    ...
    if (uploaded.length) {
      ...
      if (documentsError) {
        console.error('Supabase document record error:', documentsError);
...
app.get('/api/applications/track/:trackingNumber', async (req, res) => {
  const { data, error } = await supabase
    .from('applications')
    .select(`
      id, tracking_number, full_name, email, phone, gender, nationality,
      job_title, job_location, industry, experience, education,
       status, status_step, submitted_at, admin_notes, employer_email, employer_company,
      passport_number, has_passport, worked_qatar,
      emergency_name, emergency_relationship, emergency_country
    `)
  ...
  const [{ data: comments }, { data: documents }, { data: employer }] = await Promise.all([
    ...
    data.employer_email
      ? supabase.from('employer_profiles').select('name,company_name,photo_url,email').eq('email', data.employer_email).maybeSingle()
...
/**
 * GET /api/employers/access?email=
 * Employer access is intentionally email-based for the supplied workflow.
 */
app.get('/api/employers/access', async (req, res) => {
  const email = cleanEmail(req.query.email);
  if (!email) return res.status(400).json({ error: 'Employer email is required.' });
  ...
  const [{ data: profile, error: profileError }, { data: applications, error: applicationsError }] = await Promise.all([
    ...
    supabase
      .from('applications')
      .select('id,tracking_number,full_name,email,phone,nationality,job_title,job_location,experience,education,status,status_step,submitted_at,updated_at,employer_email,employer_company')
      .eq('employer_email', email)
      .order('submitted_at', { ascending: false }),
  ]);
  if (profileError) return res.status(500).json({ error: profileError.message });
  if (applicationsError) return res.status(500).json({ error: applicationsError.message });
  return res.json({ profile: profile || null, applications: applications || [] });
});

app.get('/api/employers/applications/:id', async (req, res) => {
  const email = cleanEmail(req.query.email);
  if (!email) return res.status(400).json({ error: 'Employer email is required.' });
...
app.put('/api/employers/applications/:id/decision', async (req, res) => {
  ...
  const status = decision === 'approved'
    ...
    : decision === 'declined' || decision === 'rejected'
      ? 'rejected'
      : requestedStatus;
  const validStatuses = ['employer_review', 'interview_scheduled', 'job_offer', 'rejected'];
  if (!email || !validStatuses.includes(status)) {
    return res.status(400).json({ error: 'Choose approve or decline and provide the employer email.' });
  }
  ...
  const statusStep = status === 'rejected' ? 0 : ({ employer_review: 3, interview_scheduled: 4, job_offer: 5 }[status] || 3);
  ...
  const note = comment || (status === 'rejected' ? 'The employer declined this application.' : 'The employer approved this application for the next stage.');
...
app.post('/api/jobs', async (req, res) => {
  try {
    ...
    const jobData = {
      ...
      contact_email: cleanEmail(body.contactEmail || body.contact_email),
...
app.get('/api/admin/employers', adminAuth, async (req, res) => {
  ...
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ employers: data || [] });
});
...
const pageAliases = {
  '/': 'index_1786088942293.html',
  '/index.html': 'index_1786088942293.html',
  '/post-job.html': 'post-job_1786088942379.html',
  '/dashboard.html': 'dashboard_1786088942415.html',
  '/apply.html': 'apply_1786088942456.html',
  '/about-us.html': 'about-us_1786088942504.html',
  '/contact.html': 'contact_1786088942543.html',
  '/career-advice.html': 'career-advice_1786088942587.html',
  '/privacy-policy.html': 'privacy-policy_1786088942634.html',
  '/terms-of-service.html': 'terms-of-service_1786088942688.html',
  '/admin.html': 'admin_1786088942745.html',
  '/apply-agent.html': 'contact_1786088942543.html',
};
...
app.listen(PORT, () => {
  ...
  if (!process.env.SUPABASE_URL) {
    console.warn('⚠  SUPABASE_URL is not set — database features will not work');
  }
});
