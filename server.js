...
// Employer login tokens are signed here, so this secret must remain stable
// across deploys/restarts. SESSION_SECRET is the preferred shared setting;
// CHAT_SECRET is supported for Render deployments that use a dedicated name.
// The fallback keeps an unconfigured local copy runnable, but tokens from it
// are intentionally not durable.
const CHAT_SECRET = process.env.SESSION_SECRET
...
function chatReadsTableMissing(error) {
  return /relation .*chat_message_reads.* does not exist|could not find the table .*chat_message_reads/i.test(error?.message || '');
}

async function getChatReadRows(messageIds) {
  ...
  const { data, error } = await supabase
    ...
    .in('message_id', ids);
  // Keep older deployments usable until the additive migration is run.
  if (error && chatReadsTableMissing(error)) return null;
  if (error) throw error;
  return data || [];
}

async function addChatReadState(messages) {
  const rows = messages || [];
  const reads = await getChatReadRows(rows.map(message => message.id));
  if (reads === null) return rows.map(message => ({ ...message, read_by: [] }));
  const readsByMessage = {};
  reads.forEach(read => {
    if (!readsByMessage[read.message_id]) readsByMessage[read.message_id] = [];
    readsByMessage[read.message_id].push(read.reader_role);
  });
  return rows.map(message => ({
    ...message,
    read_by: [...new Set(readsByMessage[message.id] || [])],
  }));
}

async function markChatMessagesRead(applicationId, readerRole) {
  ...
  if (!messages?.length) return { marked: 0, supported: true };
  ...
  const { error } = await supabase
    .from('chat_message_reads')
    .upsert(rows, { onConflict: 'message_id,reader_role' });
  if (error && chatReadsTableMissing(error)) return { marked: 0, supported: false };
  if (error) throw error;
  return { marked: rows.length, supported: true };
}

async function getChatUnreadCount(applicationId, readerRole) {
  const { data: messages, error } = await supabase
    ...
    .neq('sender_role', readerRole);
  if (error) throw error;
  const reads = await getChatReadRows((messages || []).map(message => message.id));
  if (reads === null) return 0;
  const readIds = new Set((reads || [])
    .filter(read => read.reader_role === readerRole)
...
function chatMessageData(applicationId, body, defaults) {
  ...
  return {
    ...
    sender_name: String(
      defaults.senderRole === 'applicant'
        ? (defaults.senderName || 'Applicant')
        : (body.senderName || defaults.senderName || 'FindAJob.qa')
    ).trim().slice(0, 120),
...
const FAQ_REPLIES = {
  ...
  documents: 'Your passport photo and CV are required first. If you have a valid passport, upload a clear copy now. If you need to apply for or renew your passport, you can provide the new copy later before visa processing. After an employer has reviewed your application and a genuine employment relationship has been established, the employer or a licensed recruitment partner may be able to guide you or discuss lawful support for official passport processing, but this is not guaranteed. Never pay upfront, pay anyone to guarantee a job or visa, or surrender your original identity documents.',
...
const DEFAULT_AUTOMATED_MESSAGES = [
  { message_key: 'access_helper', category: 'helper', label: 'Reference lookup helper', body: 'Forgot your reference number? Use the email address you used when applying.', active: true, sort_order: 1 },
  { message_key: 'chat_locked_helper', category: 'helper', label: 'Locked chat helper', body: 'Enter your application reference or email to securely open your chat.', active: true, sort_order: 2 },
  { message_key: 'employer_review', category: 'progress', label: 'Employer review', body: 'Your profile is now being reviewed by the employer for this role.', active: true, sort_order: 10 },
  { message_key: 'interview_scheduled', category: 'progress', label: 'Interview scheduled', body: 'Your interview has been scheduled. The employer will share the time and meeting details with you shortly.', active: true, sort_order: 20 },
  { message_key: 'job_offer', category: 'progress', label: 'Job offer', body: 'Your job offer has been issued. Please review the offer and employment contract carefully before signing.', active: true, sort_order: 30 },
  { message_key: 'visa_processing', category: 'progress', label: 'Visa processing', body: 'Your visa is now being processed. Your employer has submitted the Qatar work visa application on your behalf. We will update you when there is progress.', active: true, sort_order: 40 },
  { message_key: 'visa_approved', category: 'progress', label: 'Visa approved', body: 'Your Qatar work visa has been approved. Your entry permit and next travel instructions will be shared with you.', active: true, sort_order: 50 },
  { message_key: 'flight_prep', category: 'progress', label: 'Flight preparation', body: 'Your flight preparation is now underway. Your employer or agency will confirm your travel arrangements with you.', active: true, sort_order: 60 },
  { message_key: 'arrived', category: 'progress', label: 'Arrived in Qatar', body: 'You have been marked as arrived in Qatar. Your employer will guide you through onboarding and required registrations.', active: true, sort_order: 70 },
  { message_key: 'employed', category: 'progress', label: 'Employment started', body: 'Your employment has started. Welcome to your new role in Qatar!', active: true, sort_order: 80 },
  { message_key: 'rejected', category: 'progress', label: 'Application not selected', body: 'Your application was not selected for this opportunity. FindAJob.qa wishes you success with your next application.', active: true, sort_order: 90 },
  { message_key: 'cancelled', category: 'progress', label: 'Application cancelled', body: 'This application has been cancelled. Please contact FindAJob.qa if you believe this was a mistake.', active: true, sort_order: 100 },
];

function automatedMessagesTableMissing(error) {
  return /relation .*automated_messages.* does not exist|could not find the table .*automated_messages/i.test(error?.message || '');
}

function mergeAutomatedMessages(rows) {
  const stored = Array.isArray(rows) ? rows : [];
  const storedKeys = new Set(stored.map(message => message.message_key));
  return stored
    .concat(DEFAULT_AUTOMATED_MESSAGES.filter(message => !storedKeys.has(message.message_key)))
    .sort((left, right) => {
...
async function getAutomatedMessage(messageKey) {
  const fallback = DEFAULT_AUTOMATED_MESSAGES.find(message => message.message_key === messageKey);
  try {
    const { data, error } = await supabase
      .from('automated_messages')
      .select('message_key,category,label,body,active,sort_order')
      .eq('message_key', messageKey)
      .maybeSingle();
    if (error) {
      if (automatedMessagesTableMissing(error)) return fallback?.active === false ? null : fallback?.body || null;
      throw error;
    }
    if (!data) return fallback?.active === false ? null : fallback?.body || null;
    return data.active === false ? null : data.body;
  } catch (error) {
    if (automatedMessagesTableMissing(error)) return fallback?.active === false ? null : fallback?.body || null;
    throw error;
  }
...
const DEFAULT_CONTACT_SETTINGS = {
  ...
  whatsapp_number: '',
  call_number: '',
  whatsapp_message: 'Hello, I would like to ask about available jobs in Qatar.',
};

function contactSettingsTableMissing(error) {
  return /relation .*contact_settings.* does not exist|could not find the table .*contact_settings/i.test(error?.message || '');
}

function cleanContactSettings(body = {}) {
  return {
    setting_key: 'default',
    whatsapp_number: String(body.whatsappNumber ?? body.whatsapp_number ?? '').trim().slice(0, 40),
    call_number: String(body.callNumber ?? body.call_number ?? '').trim().slice(0, 40),
    whatsapp_message: String(body.whatsappMessage ?? body.whatsapp_message ?? DEFAULT_CONTACT_SETTINGS.whatsapp_message).trim().slice(0, 500)
      || DEFAULT_CONTACT_SETTINGS.whatsapp_message,
    updated_at: new Date().toISOString(),
  };
}

const PROGRESS_UPDATE_MESSAGES = Object.fromEntries(
  DEFAULT_AUTOMATED_MESSAGES
    .filter(message => message.category === 'progress')
    .map(message => [message.message_key, message.body]),
);
...
async function insertProgressUpdateMessage(applicationId, status, senderRole, senderName, senderEmail, comment) {
  const message = await getAutomatedMessage(status);
  if (!message) return;
  const note = String(comment || '').trim();
  const body = `${progressStatusLabel(status)}\n${message}${note ? `\n\nRecruiter note: ${note}` : ''}`;
  const { error } = await supabase.from('chat_messages').insert([{
    application_id: applicationId,
    sender_role: senderRole,
    sender_name: senderName || 'FindAJob.qa',
...
async function findApplicationByRef(trackingNumber) {
  ...
  let query = supabase
    .from('applications')
    .select('id,tracking_number,full_name,email,status,status_step,job_title,job_location,submitted_at,updated_at,employer_email,employer_company')
    .order('submitted_at', { ascending: false })
    .limit(1);
  query = value.includes('@')
    ? query.eq('email', cleanEmail(value))
    : query.eq('tracking_number', value.toUpperCase());
...
app.post('/api/applications', upload.fields([
  ...
  try {
    ...
    await supabase.from('chat_messages').insert([{
      ...
      body: `Application submitted. Reference number: ${trackingNumber}.`,
      is_encrypted: false,
      message_type: 'system',
    }]);
...
app.get('/api/applications/track/:trackingNumber', async (req, res) => {
  const lookup = String(req.params.trackingNumber || '').trim();
  let applicationQuery = supabase
    ...
    .order('submitted_at', { ascending: false })
    .limit(1);
  applicationQuery = lookup.includes('@')
    ? applicationQuery.eq('email', cleanEmail(lookup))
    : applicationQuery.eq('tracking_number', lookup.toUpperCase());
  const { data: applications, error } = await applicationQuery;
  const data = applications?.[0] || null;
  ...
  return res.json({
    ...
    job: job || null,
    messages: await addSignedChatUrls(await addChatReadState(messages)),
    presence: presence || [],
    employers: employers || [],
    unread_count: await getChatUnreadCount(data.id, 'applicant'),
  });
});
...
app.get('/api/applications/track/:trackingNumber/messages', async (req, res) => {
  try {
    ...
    if (error) return res.status(500).json({ error: error.message });
     return res.json({ messages: await addSignedChatUrls(await addChatReadState(messages)), presence: presence || [] });
  } catch (err) {
...
app.post('/api/applications/:trackingNumber/messages', upload.array('attachments', 5), async (req, res) => {
  try {
    ...
    return res.status(201).json({
      success: true,
      saved: true,
      message: (await addSignedChatUrls(await addChatReadState([data])))[0],
    });
  } catch (err) {
...
/**
 * GET /api/applications/resolve?value=...
 * Resolve a reference or application email for the public tracking gate.
 * Only non-sensitive application summary fields are returned.
 */
app.get('/api/applications/resolve', async (req, res) => {
  try {
    const value = String(req.query.value || '').trim();
    if (!value) return res.status(400).json({ error: 'Reference number or email is required.' });
    const application = await findApplicationByRef(value);
    if (!application) return res.status(404).json({ error: 'No application was found for that reference or email.' });
    return res.json({
      application: {
        id: application.id,
        ...
        status_step: application.status_step,
      },
    });
  } catch (error) {
    return res.status(500).json({ error: 'Could not look up the application.' });
  }
});

app.post('/api/applications/:trackingNumber/messages/read', async (req, res) => {
  try {
    const application = await findApplicationByRef(req.params.trackingNumber);
    if (!application) return res.status(404).json({ error: 'Application not found' });
    ...
    const employerIdentity = role === 'employer'
      ? employerFromToken(String(req.headers.authorization || '').replace(/^Bearer\s+/i, ''))
      : null;
    const admin = req.headers['x-admin-password'] === ADMIN_PASSWORD;
    if (role === 'employer' && !admin && (!employerIdentity || employerIdentity.email !== cleanEmail(application.employer_email))) {
      return res.status(403).json({ error: 'Employer access is not allowed for this application.' });
    }
    const result = await markChatMessagesRead(application.id, role);
    return res.json({ success: true, ...result });
  } catch (err) {
    return res.status(500).json({ error: err.message || 'Could not mark messages as read.' });
  }
});
  ...
  try {
    ...
    if (existing) {
      ...
      if (!identity || identity.email !== email) {
        return res.status(409).json({ error: 'This email is already in use. Please log in.' });
      }
    }
    ...
    const profileQuery = existing
      ? supabase.from('employer_profiles').update(profile).eq('id', existing.id)
      : supabase.from('employer_profiles').insert([profile]);
...
app.get('/api/employers/access', requireEmployer, async (req, res) => {
  ...
  const employerApplications = await Promise.all((applications || []).map(async application => ({
    ...
    unread_count: await getChatUnreadCount(application.id, 'employer'),
  })));
  return res.json({
...
app.put('/api/employers/applications/:id/decision', requireEmployer, async (req, res) => {
  ...
  const validStatuses = [
    'employer_review', 'interview_scheduled', 'job_offer', 'visa_processing',
  ...
  const statusStep = ['rejected', 'cancelled'].includes(status) ? 0 : ({ employer_review: 3, interview_scheduled: 4, job_offer: 5, visa_processing: 6, visa_approved: 7, flight_prep: 8, arrived: 9, employed: 10 }[status] || 3);
  ...
  try {
    await insertProgressUpdateMessage(req.params.id, status, 'employer', employerName || req.employer.name, email, comment);
  } catch (messageError) {
...
app.put('/api/admin/applications/:id/status', adminAuth, async (req, res) => {
  ...
  try {
    await insertProgressUpdateMessage(
      req.params.id,
      status,
      'admin',
      changedBy || 'FindAJob.qa Support',
      null,
      note
    );
  } catch (messageError) {
...
// These values are read by index.html and apply.html. Only the admin routes
// below can change them.
app.get('/api/contact-settings', async (req, res) => {
  const { data, error } = await supabase
    .from('contact_settings')
    .select('whatsapp_number,call_number,whatsapp_message')
    .eq('setting_key', 'default')
    .limit(1);
  if (error) {
    if (contactSettingsTableMissing(error)) return res.json({ contactSettings: DEFAULT_CONTACT_SETTINGS });
    return res.status(500).json({ error: error.message });
  }
  return res.json({ contactSettings: data?.[0] || DEFAULT_CONTACT_SETTINGS });
});

app.get('/api/admin/contact-settings', adminAuth, async (req, res) => {
  const { data, error } = await supabase
    .from('contact_settings')
    .select('*')
    .eq('setting_key', 'default')
...
app.put('/api/admin/contact-settings', adminAuth, async (req, res) => {
  const settings = cleanContactSettings(req.body || {});
  const { data, error } = await supabase
    .from('contact_settings')
    ...
    .single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true, contactSettings: data });
});
...
app.get('/api/automated-messages', async (req, res) => {
  ...
  if (error) {
    if (automatedMessagesTableMissing(error)) {
      return res.json({ messages: DEFAULT_AUTOMATED_MESSAGES.filter(message => message.active) });
    }
    return res.status(500).json({ error: error.message });
  }
  return res.json({ messages: mergeAutomatedMessages(data).filter(message => message.active) });
});

app.get('/api/admin/automated-messages', adminAuth, async (req, res) => {
  const { data, error } = await supabase
    .from('automated_messages')
    .select('*')
    ...
    .order('sort_order', { ascending: true });
  if (error) {
    if (automatedMessagesTableMissing(error)) return res.json({ messages: DEFAULT_AUTOMATED_MESSAGES });
    return res.status(500).json({ error: error.message });
  }
  return res.json({ messages: mergeAutomatedMessages(data) });
});

function cleanAutomatedMessage(body = {}, existingKey = '') {
  return {
    message_key: String(body.messageKey || body.message_key || existingKey || '')
      .trim().toLowerCase().replace(/[^a-z0-9_-]/g, '-'),
    category: ['progress', 'helper'].includes(String(body.category || '').toLowerCase())
...
app.post('/api/admin/automated-messages', adminAuth, async (req, res) => {
  const message = cleanAutomatedMessage(req.body);
  if (!message.message_key || !message.label || !message.body) {
    return res.status(400).json({ error: 'Message key, label, and message body are required.' });
  }
  const { data, error } = await supabase.from('automated_messages').upsert([message], { onConflict: 'message_key' }).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.status(201).json({ success: true, message: data });
});

app.put('/api/admin/automated-messages/:messageKey', adminAuth, async (req, res) => {
  const message = cleanAutomatedMessage(req.body, req.params.messageKey);
  delete message.message_key;
  const { data, error } = await supabase.from('automated_messages').update(message).eq('message_key', req.params.messageKey).select().single();
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true, message: data });
});

app.delete('/api/admin/automated-messages/:messageKey', adminAuth, async (req, res) => {
  const { error } = await supabase.from('automated_messages').delete().eq('message_key', req.params.messageKey);
  if (error) return res.status(500).json({ error: error.message });
  return res.json({ success: true });
});
